...
Tehai_Pattern_Source::Tehai_Pattern_Source() {
	reset();
}

void Tehai_Pattern_Source::reset() {
	mentu_hai.erase(mentu_hai.begin(),mentu_hai.end());
	head_hai.erase(head_hai.begin(),head_hai.end());
	tatu_hai.erase(tatu_hai.begin(),tatu_hai.end());
	koritu1_hai.erase(koritu1_hai.begin(),koritu1_hai.end());
	koritu2_hai.erase(koritu2_hai.begin(),koritu2_hai.end());
}

Tehai_Analyzer_Basic::Tehai_Analyzer_Basic(){
	reset_tehai_analyzer_basic();
}

Tehai_Analyzer::Tehai_Analyzer() : Tehai_Analyzer_Basic() {
}
...
void Tehai_Analyzer::reset_tehai_analyzer() {
	reset_tehai_analyzer_basic();
	pattern_flag = 0;
...
int Tehai_Analyzer_Basic::get_tehai_num() const {
	return (num_and_flags & 0x000000F0) >> 4; 
}

void Tehai_Analyzer_Basic::set_tehai_num(const int num) {
	assert(num <= 15); // 有効牌を得るために、14枚の手牌にさらに1枚足すことがある。
	num_and_flags = (num_and_flags & 0xFFFFFF0F) + (uint32_t(num) << 4);
}

void Tehai_Analyzer_Basic::add_tehai_num(const int num) {
	num_and_flags += (uint32_t(num) << 4);
}

void Tehai_Analyzer_Basic::reduce_tehai_num(const int num) {
	num_and_flags -= (uint32_t(num) << 4);
}

int Tehai_Analyzer_Basic::get_fuuro_num() const {
	return (num_and_flags & 0x00000F00) >> 8; 
}

void Tehai_Analyzer_Basic::set_fuuro_num(const int num) {
	assert(num <= 4);
	num_and_flags = (num_and_flags & 0xFFFFF0FF) + (uint32_t(num) << 8);
}

void Tehai_Analyzer_Basic::add_fuuro_num(const int num) {
	num_and_flags += (uint32_t(num) << 8);
}

void Tehai_Analyzer_Basic::reduce_fuuro_num(const int num) {
	num_and_flags -= (uint32_t(num) << 8);
}

int Tehai_Analyzer_Basic::get_ankan_num() const {
	return (num_and_flags & 0x0000F000) >> 12; 
}

void Tehai_Analyzer_Basic::set_ankan_num(const int num) {
	assert(num <= 4);
	num_and_flags = (num_and_flags & 0xFFFF0FFF) + (uint32_t(num) << 12);
}

void Tehai_Analyzer_Basic::add_ankan_num(const int num) {
	num_and_flags += (uint32_t(num) << 12);
}

int Tehai_Analyzer_Basic::get_mentu_shanten_num() const {
	return (num_and_flags & 0x000F0000) >> 16;
}

int Tehai_Analyzer_Basic::get_titoi_shanten_num() const {
	return (num_and_flags & 0x00F00000) >> 20;
}

int Tehai_Analyzer_Basic::get_agari_shanten_num() const {
...
void Tehai_Analyzer_Basic::set_titoi_shanten_num(const int num) {
...
void Tehai_Analyzer_Basic::delete_hai(const int hai) {
	reduce_tehai_num(1);
	if(hai%10 == 0) {
		assert(hai != 0);
...
void Tehai_Analyzer_Basic::add_daiminkan(const int hai, const int hai2) {
	assert(hai%10 != 0);
...
void Tehai_Analyzer_Basic::change_pon_to_kakan(const int hai, const int hai2) {
    assert(hai%10 != 0);
	assert(hai == haikind(hai2));
...
void Tehai_Analyzer_Basic::add_ankan(const int hai, const int hai2) {
	assert(hai%10 != 0);
	assert(hai == haikind(hai2));
...
void Tehai_Analyzer_Basic::set_fuuro(const Fuuro_Vector& fuuro){
	set_fuuro_num(0);
	set_ankan_num(0);
...
void Tehai_Analyzer_Basic::pattern_push_func(const int my_pid_new, const Game_State& game_state, const Hai_Array& tehai_kind, Tehai_Pattern_Source& tps) {
}
...
void Tehai_Analyzer::pattern_titoi_push_func(Tehai_Pattern_Source& tps) {
...
template <class Agari_Vector> void Tehai_Analyzer_Basic::tenpai_check(const int my_pid_new, const Game_State& game_state, const Hai_Array& tehai_kcp, Hai_Array& tehai_tate_cut, Hai_Array& tehai_tmp, Agari_Vector& agariv){
	int rest = 0;
	...
	if(rest==0){
		...
		for(int hai=0;hai<38;hai++){
			if(tehai_tmp[hai] && using_haikind_num(hai)!=4){
				//printf("tanki:%d\n",hai);
				set_tenpai_flag(1);
				agari_push_func(my_pid_new, game_state, tehai_kcp, tehai_tate_cut, tehai_tmp, hai, MT_TANKI, HT_DEFAULT, agariv);
			}
		}
	}else if(rest==2){
		for(int hai=0;hai<38;hai++){
			if(tehai_tmp[hai]==2 && using_haikind_num(hai)!=4){
				//printf("syabo:%d\n",hai);
				set_tenpai_flag(1);
				agari_push_func(my_pid_new, game_state, tehai_kcp, tehai_tate_cut, tehai_tmp, hai, MT_SHABO, HT_DEFAULT, agariv);
			}
		}
		for(int hai=0;hai<30;hai++){
			if(tehai_tmp[hai]&&tehai_tmp[hai+1]){
				if(hai%10==1){
					...
					if(using_haikind_num(hai+2)!=4){
						set_tenpai_flag(1);
						agari_push_func(my_pid_new, game_state, tehai_kcp, tehai_tate_cut, tehai_tmp, hai+2, MT_PENCHAN, HT_DEFAULT, agariv);
					}
				}else if(hai%10==8){
					//printf("penchan:%d\n",hai-1);
					if(using_haikind_num(hai-1)!=4){
						set_tenpai_flag(1);
						agari_push_func(my_pid_new, game_state, tehai_kcp, tehai_tate_cut, tehai_tmp, hai-1, MT_PENCHAN, HT_DEFAULT, agariv);
					}
				}else{
					//printf("ryanmen:%d:%d\n",hai-1,hai+2);
					if(using_haikind_num(hai-1)!=4){
						set_tenpai_flag(1);
						agari_push_func(my_pid_new, game_state, tehai_kcp, tehai_tate_cut, tehai_tmp, hai-1, MT_RYANMEN, HT_DEFAULT, agariv);
					}
					if(using_haikind_num(hai+2)!=4){
						set_tenpai_flag(1);
						agari_push_func(my_pid_new, game_state, tehai_kcp, tehai_tate_cut, tehai_tmp, hai+2, MT_RYANMEN, HT_DEFAULT, agariv);
					}
				}
			}
			if(tehai_tmp[hai]&&tehai_tmp[hai+2]){
				if(hai%10!=9){
					...
					if(using_haikind_num(hai+1)!=4){
						set_tenpai_flag(1);
						agari_push_func(my_pid_new, game_state, tehai_kcp, tehai_tate_cut, tehai_tmp, hai+1, MT_KANCHAN, HT_DEFAULT, agariv);
					}
				}
			}
		}
...
void Tehai_Analyzer_Basic::cut_koritu(const int my_pid_new, const Game_State& game_state, const Hai_Array& tehai_kcp, Hai_Array& tehai_tate_cut, Hai_Array& tehai_tmp, int mentu_num, int head_num, int kouho_num, int start, Tehai_Pattern_Source& tps){
...
void Tehai_Analyzer_Basic::analyze_koritu(const int my_pid_new, const Game_State& game_state, const Hai_Array& tehai_kcp, Hai_Array& tehai_tate_cut, Hai_Array& tehai_tmp, int mentu_num, int head_num, int kouho_num, Tehai_Pattern_Source& tps){
	if(head_num==0){
		for(int hai=0;hai<38;hai++){
			if(tehai_tmp[hai]>=1){
...
template <class Agari_Vector> void Tehai_Analyzer_Basic::analyze_tatu(const int my_pid_new, const Game_State& game_state, const Hai_Array& tehai_kcp, Hai_Array& tehai_tate_cut, Hai_Array& tehai_tmp, Tehai_Pattern_Source& tps, Agari_Vector& agariv){
	int head_num_tmp = 0;
...
template <class Agari_Vector> void Tehai_Analyzer_Basic::cut_syuntu(const int my_pid_new, const Game_State& game_state, const Hai_Array& tehai_kcp, Hai_Array& tehai_tate_cut, Hai_Array& tehai_tmp, int start, Tehai_Pattern_Source& tps, Agari_Vector& agariv){
...
template <class Agari_Vector> void Tehai_Analyzer_Basic::cut_kotu(const int my_pid_new, const Game_State& game_state, const Hai_Array& tehai_kcp, Hai_Array& tehai_tmp, int start, Tehai_Pattern_Source& tps, Agari_Vector& agariv){
	for(int hai=start;hai<38;hai++){
		if(tehai_tmp[hai]>=3){
...
void Tehai_Analyzer_Basic::titoi_cut_koritu(const Hai_Array& tehai_kcp, Hai_Array& tehai_tmp, int start, Tehai_Pattern_Source& tps){
	for(int hai=start;hai<38;hai++){
...
void Tehai_Analyzer_Basic::titoi_cut_head(const Hai_Array& tehai_kcp, Hai_Array& tehai_tmp, int start, Tehai_Pattern_Source& tps){
	for(int hai=start;hai<38;hai++){
		if(tehai_tmp[hai]>=2){
...
template <class Agari_Vector> void Tehai_Analyzer_Basic::titoi_shanten(const int my_pid_new, const Game_State& game_state, const Hai_Array& tehai_kcp, Hai_Array& tehai_tmp, Tehai_Pattern_Source& tps, Agari_Vector& agariv){
	int head_num = 0;
	int koritu_num = 0;
	for(int hai=0;hai<38;hai++){
		if(tehai_kcp[hai]>=2){
	...
	if(head_num==7){
		...
		for(int hai=0;hai<38;hai++){
			if(tehai_kcp[hai]==1){
				set_tenpai_flag(1);
				agari_push_func(my_pid_new, game_state, tehai_kcp, tehai_kcp, tehai_kcp, hai, MT_TANKI, HT_TITOI, agariv);
			}
		}
	}
...
template void Tehai_Analyzer_Basic::analyze_tehai<boost::container::static_vector<Agari_Info, 10>>(const int my_pid_new, const Game_State& game_state, boost::container::static_vector<Agari_Info, 10>& agariv);
template void Tehai_Analyzer_Basic::analyze_tehai<boost::container::static_vector<Agari_Calc, MAX_AGARI_NUM_PER_THREAD>>(const int my_pid_new, const Game_State& game_state, boost::container::static_vector<Agari_Calc, MAX_AGARI_NUM_PER_THREAD>& agariv);
template void Tehai_Analyzer_Basic::analyze_tehai<bool>(const int my_pid_new, const Game_State& game_state, bool& agariv);


void Tehai_Analyzer_Basic::analyze_tenpai(const int my_pid_new, const Game_State& game_state) {
	bool dummy = false;
...
template <class Agari_Vector> void Tehai_Analyzer_Basic::agari_push_func(
	...
	Agari_Info agari_tmp = calc_agari(31 + game_state.bakaze, 31 + game_state.player_state[pid].jikaze, tkcp, ttc, tt, fuuro_kind, mh, mt, ht);
...
void Tehai_Analyzer_Basic::agari_push_func(
	...
	if (agariv.size() < MAX_AGARI_NUM_PER_THREAD) {
		...
		Agari_Info_Detail agari_tmp = calc_agari_detail(
			31 + game_state.bakaze, 31 + game_state.player_state[pid].jikaze, tkcp, ttc, tt, fuuro_kind, mh, mt, ht
		);
...
int find_hai_out_ta(const Hai_Array& tehai, const Tehai_Analyzer_Basic& tehai_analyzer) {
	for (int hai = 1; hai < 38; hai++) {
...
int koritu_needless_num(const Hai_Array& tehai, const Game_State& game_state, const int pid, const Hai_Array hai_visible_all){
	int res = 0;
	std::vector<int> cand;
	for(int hc=0;hc<3;hc++){
		if(tehai[hc*10+1]==1 && tehai[hc*10+2]==0 && tehai[hc*10+3]==0){
			cand.push_back(hc*10+1);
		}
		if(tehai[hc*10+9]==1 && tehai[hc*10+8]==0 && tehai[hc*10+7]==0){
			cand.push_back(hc*10+9);
		}
	}
	for(int hai=31;hai<38;hai++){
		if(tehai[hai]==1){
			if (hai == 31 + game_state.player_state[pid].jikaze && hai_visible_all[hai] < 2){
				continue;
			}else if(35 <= hai && hai_visible_all[hai] < 2) {
				continue;
			}else{
				cand.push_back(hai);
			}
...
int koritu_needless_num_ta(const Tehai_Analyzer_Basic& tehai_analyzer, const Game_State& game_state, const int pid, const Hai_Array hai_visible_all) {
	Hai_Array tehai = {};
	for(int hai=1;hai<38;hai++) {
		tehai[hai] = tehai_analyzer.count_hai(hai);
	}
	return koritu_needless_num(tehai, game_state, pid, hai_visible_all);
}

void get_tehai_ta(const Tehai_Analyzer_Basic& tehai_analyzer, int tehai[38]) {
	tehai[0] = 0;
	for(int hai=1;hai<38;hai++) {
		tehai[hai] = tehai_analyzer.count_hai(hai);
	}
...
void get_tehai_kcp_ta(const Tehai_Analyzer_Basic& tehai_analyzer, int tehai_kcp[38]) {
