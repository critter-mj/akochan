...
void Selector::set_selector(const Moves& game_record, const int my_pid, const Tactics& tactics) {
	...
	const int tsumo_num_exp = std::min(cal_tsumo_num_exp(my_pid, game_state, 1, tenpai_prob_array), cal_tsumo_num_DP(game_record, my_pid)); // to do 繝・・ｽ・ｽ縺ｮ譎ゅ→蜑ｯ髴ｲ縺ｮ譎ゅ〒dahai_inc縺ｯ逡ｰ縺ｪ繧九→諤昴ｏ繧後ｋ縲・
	...
	/* get_agari_shanten_num(chi_flag, cand_hai); 縺ゅ∪繧雁ｿ・・ｽ・ｽ縺ｧ縺ｪ縺・・ｽ・ｽ繧ゅ＠繧後↑縺・・ｽE縺ｧ菫晉蕗縲・*/
	...
	// 蜑ｯ髴ｲ譎ゑｿｽE縺ｿget_fuuro_agari_shanten_num縺ｮ蜀・・ｽ・ｽ縺ｮset_agari_shanten_num縺悟他縺ｰ繧後ｋ縲ゅ♀縺昴ｉ縺丞撫鬘鯉ｿｽE辟｡縺・・ｽ・ｽE
	...
	if (DP_flag) {
		...
		if (current_action["type"] == "tsumo") {
			...
			fuuro_choice_tmp.fuuro_action_type = AT_FUURO_PASS;
			fuuro_choice_tmp.pt_exp_total = std::max(tehai_calculator.get_ten_exp(cn_fuuro_neg, gn_fuuro_neg, tsumo_num_DP), not_agari_value); // to do 蜈・・ｽEnot_agari_value縺ｮ縺九ｏ繧翫↓passive_value縺縺｣縺溘・
			fuuro_choice.push_back(fuuro_choice_tmp);

			if (current_action["type"] == "dahai" && count_tsumo_num_all(game_record) < 70) { // 繝上う繝・・ｽ・ｽ迚後ｒ繝包ｿｽE繝ｭ縺励↑縺・・ｽ・ｽ繧・ｿｽE蜃ｦ逅・
		...
		tehai_calculator.calc_agari_prob(tsumo_num_exp, other_end_value, game_state, kyoku_end_pt_exp); // ako 縺ｫ蜷医ｏ縺帙※縺・・ｽ・ｽ縺後・other_end_value 縺ｧ繧医＞縺九・
		if (out_console) {
		...
		if (current_action["type"] == "tsumo") {
						...
						if (tactics.do_kan_ordinary && count_tsumo_num_all(game_record) < 70) {
							...
							for (int acn = tsumo_edge_loc[1]; acn < tsumo_edge_loc[2]; acn++) {
								...
								if (ac_tmp.hai_out == hai_out && (ac_tmp.action_type == AT_ANKAN || ac_tmp.action_type == AT_KAKAN)) {
									...
									// other_end_value 縺ｯ蠕碁隗｣譫撰ｿｽE蝓ｺ貅門､縺ｨ縺励※逕ｨ縺・・ｽ・ｽ繧ゑｿｽE縺ｪ縺ｮ縺ｧ縲｛ther_end_value_kan縺ｫ螟峨∴縺ｪ縺・・ｽ・ｽ縺・・ｽ・ｽ繧医＞縺ｯ縺壹・
									// dahai_inc 繧・縺ｫ縺吶∋縺阪°縺ｯ隕∵､懆ｨ弱・
			...
			if (2 <= get_other_fuuro_num_max(my_pid, game_state)) {
				// 髯阪ｊ縺ｫ髢｢縺励※縲√←縺ｮ繧医≧縺ｪ譚｡莉ｶ繧呈治逕ｨ縺励◆縺ｨ縺阪↓蠑ｷ縺上↑繧九°縺ｯ隕∵､懆ｨｼ
				for (int i = 0; i < hai_choice.size(); i++) {
			...
			if (count_tsumo_num_all(game_record) < 70) { // 繝上う繝・・ｽ・ｽ迚後ｒ繝包ｿｽE繝ｭ縺励↑縺・・ｽ・ｽ繧・ｿｽE蜃ｦ逅・	
				const std::array<int, 3>& fuuro_edge_loc = tehai_calculator_work.get_const_fuuro_edge_loc(cn_fuuro_neg, gn_fuuro_neg);
				for (int acn = fuuro_edge_loc[1]; acn < fuuro_edge_loc[2]; acn++) {
					const Tehai_Action& ac_tmp = tehai_calculator_work.cand_graph_sub_fuuro_work[fuuro_edge_loc[0]][acn];
					//ac_tmp.out_info();
					if (ac_tmp.action_type != AT_TSUMO) {
						if (ac_tmp.hai != current_hai) {
							...
							for (int i = 0; i < 3; i++) { // to do. sarashi_hai繧致ector縺ｫ螟峨∴繧九・
							...
							// to do tehai_cal莉･螟厄ｿｽE繧ゑｿｽE縺ｮtsumo_num繧・蠑輔＞縺溷､縺ｧ險育ｮ励・
							fuuro_choice.push_back(fuuro_choice_tmp);
						}
					}
				}
