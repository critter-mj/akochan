...
std::array<std::array<std::array<std::array<float, 12>, 14>, 4>, 4> cal_kyoku_end_pt_exp(const Moves& game_record, const Game_State& game_state, const int my_pid, const bool reach_mode) {
    ...
    const int kyoku_first = tactics_json[my_pid]["kyoku_first"].int_value();
    const int kyoku_mod = kyoku_first + game_state.bakaze*4 + game_state.kyoku - 1;
...
void Selector::set_selector(const Moves& game_record, const int my_pid, const Tactics& tactics) {
	...
	if (DP_flag) {
		...
		if (current_action["type"] == "tsumo") {
			...
			const std::array<bool, 38> furiten_flags = get_furiten_flags(game_record, my_pid);
			...
			for (int cn = 0; cn < tehai_calculator.in0num; cn++) {
				for (int gn = 0; gn < tehai_calculator.group_size(cn); gn++) {
					if (is_ts_fuuro_consistent(tehai_calculator.get_const_ta_cgn(cn, gn).tehai_state, game_state.player_state[my_pid].fuuro)) {
						...
						if (tactics_json[my_pid]["betaori_est"] == "instant") {
							...
							hai_choice_tmp.pt_exp_after_ori = betaori.betaori_exp;
						} else if (tactics_json[my_pid]["betaori_est"] == "ako") {
							...
							std::cout << "betaori_est_error" << std::endl;
							assert(false);
						}
						hai_choice.push_back(hai_choice_tmp);
						// to do カン
